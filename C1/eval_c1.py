...
def neighbor_exists(board, row, col):   # returns true if neighbor exists, else fasle
        return board[row][col] == 1
    
def count_neighbors(board, row, col):   # returns number of neighbors given a row and column
    num_neighbors = 0
    rows = len(board)
    cols = len(board[0])
        
    if row != 0:                # not on top row
        if neighbor_exists(board, row - 1, col):          # check space directly above
            num_neighbors += 1
        if col != 0:            # not in first col and top row
            if neighbor_exists(board, row - 1, col - 1):  # check top left diag
                num_neighbors += 1
        if col != (cols - 1):   # not in last col and top row
            if neighbor_exists(board, row - 1, col + 1):  # check top right diag
                num_neighbors += 1
                
    if row != (rows - 1):       # not in bot row
        if neighbor_exists(board, row + 1, col):          # check space directly below
            num_neighbors += 1
        if col != 0:            # not in first col and bot row
            if neighbor_exists(board, row + 1, col - 1):  # check bot left diag
                num_neighbors += 1
        if col != (cols - 1):   # not in last col and top row
            if neighbor_exists(board, row + 1, col + 1):  # check bot right diag
                num_neighbors += 1

    if col != 0:                # not on first col
        if neighbor_exists(board, row, col - 1):          # check space directly left
            num_neighbors += 1
                
    if col != (cols - 1):       # not on last col
        if neighbor_exists(board, row, col + 1):          # check space directly right
            num_neighbors += 1
    return num_neighbors
            
        
def revive(board):  # returns list of cells to revive
    revive_list = []
    rows = len(board)
    cols = len(board[0])
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if board[i][j] != 1 and count_neighbors(board, i, j) == 3:
                revive_list.append((i, j))
    return revive_list
                

def kill(board):    # returns list of cells to kill
    kill_list = []
    rows = len(board)
    cols = len(board[0])
...
def update_board(board, k_list, r_list):    # updates the board according to the lists
    if len(k_list) != 0:
        for cell in k_list:
...
class Game:
    ...
    def update(self):
        ...
        board = update_board(board, kill(board), revive(board))
